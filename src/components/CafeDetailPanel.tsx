...
function getCurrentOpeningStatus(hours: unknown): {
  isOpen: boolean;
  closingSoon: boolean;
  ...
  if (
    typeof hours !== "string" ||
    hours.toLowerCase() === "true" ||
    hours.toLowerCase() === "false"
  ) {
    return {
      isOpen: false,
      closingSoon: false,
    ...
    if (appliesToday) {
      ...
      if (timeMatch) {
        ...
        const adjustedCurrentTime =
          currentTime < openTime && closeTime > 24 * 60
            ? currentTime + 24 * 60
            : currentTime;

        if (
          adjustedCurrentTime >= openTime &&
          adjustedCurrentTime < closeTime
        ) {
          // Currently open
          const minutesUntilClose = closeTime - adjustedCurrentTime;
          const closingSoon = minutesUntilClose <= 60; // Within 1 hour
          const closingTimeStr = `${String(closeHour).padStart(
            2,
            "0"
          )}:${String(closeMin).padStart(2, "0")}`;
          return {
            isOpen: true,
            closingSoon,
          ...
          // Not yet open
          const openingTimeStr = `${String(openHour).padStart(2, "0")}:${String(
            openMin
          ).padStart(2, "0")}`;
          return {
            isOpen: false,
...
export default function CafeDetailPanel({
  ...
  const [photoInputMode, setPhotoInputMode] = useState<"upload" | "url">(
    "upload"
  );
  const [photoUrlInput, setPhotoUrlInput] = useState("");
  const [isValidatingPhotoUrl, setIsValidatingPhotoUrl] = useState(false);

  // View mode state for in-panel navigation
  const [viewMode, setViewMode] = useState<"main" | "gallery">("main");
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        ...
        e.key === "ArrowRight" &&
        selectedPhotoIndex < cafePhotos.length - 1
      ) {
        setSelectedPhotoIndex(selectedPhotoIndex + 1);
        setIsPhotoLoading(true);
      }
  ...
  }, [selectedPhotoIndex, cafePhotos.length]);
  ...
  const fetchAddress = useCallback(
    (cafeId: string, lat: number, lon: number) => {
  ...
  useEffect(() => {
    ...
    // Reset view mode when cafe changes
    setViewMode("main");
  }, [cafe]);
  ...
  const handleConfirmUpload = useCallback(async () => {
    ...
    try {
      ...
      setToastMessage(
        language === "id" ? "Gagal mengupload foto" : "Failed to upload photos"
      );
      setToastType("error");
      setShowToast(true);
    } finally {
  ...
  // Add photo via URL - downloads and re-uploads to Cloudinary
  const handleAddPhotoUrl = useCallback(async () => {
    ...
    try {
      ...
      // Check if this URL already exists (by comparing the URL)
      if (cafePhotos.includes(cloudinaryUrl)) {
        setToastMessage(
          language === "id"
            ? "Foto ini sudah ada di galeri"
            : "This photo is already in gallery"
        );
      ...
      // Add to photos
      const updatedPhotos = [...cafePhotos, cloudinaryUrl];
  ...
  const handleToggleFavorite = () => {
    if (cafe) {
      ...
      if (newState) {
        setToastMessage(
          language === "id"
            ? `${cafe.name} ditambahkan ke favorit`
            : `${cafe.name} added to favorites`
        );
        setToastType("success");
      } else {
        setToastMessage(
          language === "id"
            ? `${cafe.name} dihapus dari favorit`
            : `${cafe.name} removed from favorites`
        );
        setToastType("info");
      }
      setShowToast(true);
  ...
  const t = {
    id: {
      ...
      beFirstPhoto: "Jadilah yang pertama menambahkan foto!",
        ...
        {isOpen && cafe && (
          <>
            ...
            <motion.div
              ...
              rounded-t-3xl
              /* Desktop: positioned on right side, shrink-wrap to content with max-height */
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0 overflow-hidden">
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {(
                            cafe as unknown as { isPermanentlyClosed?: boolean }
                          ).isPermanentlyClosed && (
                    ...
                    >
                      ...
                      >
                        ...
                        {photoInputMode === "upload" && (
                          <label
                            className={`
                          flex items-center justify-center gap-2 py-4 rounded-xl border-2 border-dashed cursor-pointer transition-all
                          ${
                            pendingPhotos.length > 0 || isUploadingPhotos
                              ? "opacity-50 cursor-not-allowed"
                              : ""
                          }
                          ${
                            isDarkMode
                              ? "border-gray-600 hover:border-primary-500 bg-gray-800/50 hover:bg-gray-800"
                              : "border-gray-300 hover:border-primary-500 bg-white hover:bg-gray-50"
                          }
                        `}
                          >
                            <Icon
                            ...
                            <input
                              ...
                              disabled={
                                isUploadingPhotos || pendingPhotos.length > 0
                              }
                            />
                          </label>
                        )}
                        ...
                        {photoInputMode === "url" && (
                          <div className="flex gap-2">
                            <input
                              ...
                              className={`
                              flex-1 px-4 py-3 rounded-xl border-2 text-sm transition-all
                              ${
                                isDarkMode
                            ...
                            <button
                              ...
                              disabled={
                                isValidatingPhotoUrl || !photoUrlInput.trim()
                              }
                              className={`
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {cafePhotos.map((photo, index) => (
                            <motion.div
                              ...
                              className="aspect-square rounded-xl overflow-hidden bg-gray-800 cursor-pointer group relative"
                              onClick={() => {
                      ...
                      `}
                        >
                          ...
                          />
                          <p className="text-sm font-medium">{text.noPhotos}</p>
                        ...
                        {selectedPhotoIndex !== null && (
                          ...
                          >
                            ...
                            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 bg-white/10 text-white rounded-full text-sm backdrop-blur-sm">
                              {selectedPhotoIndex + 1} / {cafePhotos.length}
                            </div>
                      ...
                      <div className="px-4 pt-2 pb-3">
                        {cafePhotos.length > 0 ? (
                          ...
                          >
                            {/* Banner Image */}
                            <div
                              className="aspect-[16/9] md:aspect-[21/9] rounded-2xl overflow-hidden bg-gray-800 cursor-pointer group"
                              onClick={() => setViewMode("gallery")}
                            >
                              <img
                                src={getThumbnailUrl(cafePhotos[0])}
                                alt={`${cafe.name} banner`}
                                className="w-full h-full object-cover transition-transform group-hover:scale-105"
                              />
                              {/* Overlay with photo count */}
                              <div className="absolute inset-0 bg-gradient-to-t rounded-b-2xl from-black/60 via-transparent to-transparent flex items-end justify-between p-4">
                          ...
                          /* No Photos - Show Add Photo CTA */
                          <motion.div
                            initial={{ opacity: 0, y: 10 }}
                            animate={{ opacity: 1, y: 0 }}
                            onClick={() => setViewMode("gallery")}
                            className={`
                            text-center py-6 rounded-2xl border-2 border-dashed cursor-pointer transition-all
                            ${
                              isDarkMode
                                ? "border-gray-700 text-gray-500 hover:border-primary-500/50 hover:bg-gray-800/50"
                                : "border-gray-300 text-gray-400 hover:border-primary-500/50 hover:bg-gray-50"
                            }
                          `}
                          >
                            <Icon
                      ...
                      <div className="px-4 pb-4 space-y-3">
                        ...
                        {(() => {
                          ...
                          const isValidHours =
                            ...
                            hours !== "true" &&
                            hours !== "false";
                          const parsedHours = isValidHours
                            ? parseOpeningHours(hours, language)
                            : [];

                          // Also check instagram field - sometimes opening hours end up there
                          const cafeData = cafe as unknown as Record<
                            string,
                            unknown
                          >;
                          const instagramValue = cafeData.instagram;
                          const isInstagramActuallyHours =
                            instagramValue &&
                            typeof instagramValue === "string" &&
                          ...
                          const displayHours =
                            parsedHours.length > 0
                              ? parsedHours
                              : isInstagramActuallyHours
                              ? parseOpeningHours(instagramValue, language)
                              : [];
                          ...
                          const currentDayAbbr =
                            dayAbbreviations[currentDayIndex];
                          ...
                          const isToday = (originalDays: string) => {
                            ...
                            // Handle range: Mo-Fr
                            const rangeMatch = originalDays.match(
                              /^([A-Z][a-z])-([A-Z][a-z])$/
                            );
                            if (rangeMatch) {
                              const startDay = dayAbbreviations.indexOf(
                                rangeMatch[1]
                              );
                              const endDay = dayAbbreviations.indexOf(
                                rangeMatch[2]
                              );
                              if (startDay <= endDay) {
                                return (
                    ...
                    `}
                        ...
                        {(() => {
                          const cafeData = cafe as unknown as Record<
                            ...
                            unknown
                          >;
                          let instagramValue = cafeData.instagram as
                            | string
                            | undefined;
                          ...
                          const isValidInstagram =
                            ...
                            !instagramValue.includes(":") &&
                            !instagramValue.match(/\d{1,2}[:.,-]\d{2}/) &&
                            (instagramValue.startsWith("@") ||
                              instagramValue.match(/^[a-zA-Z0-9._]+$/));
                      ...
                      `}
                        ...
                        {(() => {
                          const cafeData = cafe as unknown as Record<
                            ...
                            unknown
                          >;
                          const priceRangeValue = cafeData.priceRange as
                            | string
                            | undefined;

                          if (
                            !priceRangeValue ||
                            !["low", "medium", "high"].includes(priceRangeValue)
                          ) {
                            return null;
                          }
                        ...
                        {(cafe.hasWifi ||
                          ...
                          (cafe.smokingPolicy &&
                            ...
                            ["yes", "no", "outside", "separated"].includes(
                              cafe.smokingPolicy.toLowerCase()
                            ))) && (
                          <motion.div
                            custom={4}
                            variants={itemVariants}
                      ...
                      `}
                              ...
                              {/* Good for Working */}
                              {(cafe as unknown as { goodForWorking?: boolean })
                                .goodForWorking && (
                                <div
                                  className={`
                        inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium
                      ...
                      `}
                                  >
                                    ...
                                    {cafe.smokingPolicy.toLowerCase() ===
                                      "yes" && text.smokingYes}
                                    {cafe.smokingPolicy.toLowerCase() ===
                                      "outside" && text.smokingOutside}
                                    {cafe.smokingPolicy.toLowerCase() ===
                                      "separated" && text.smokingSeparated}
                                  </div>
                                )}

                              {/* Brand - filter out Instagram handles */}
                              {cafe.brand &&
                                !cafe.brand.startsWith("@") &&
                                !cafe.brand
                                  .toLowerCase()
                                  .includes("instagram") && (
                                  <div
              ...
              >
                ...
                {userLocation && onShowRoute && (
                  ...
                  >
                    <Icon
                      icon={
                        isShowingRoute ? "mdi:map-marker-path" : "mdi:routes"
                      }
                      className="w-5 h-5"
                    />
                    <span>
